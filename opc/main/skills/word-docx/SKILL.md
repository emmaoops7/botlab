...
生成中文 Word 文档时，**必须设置中文字体**，否则中文会显示为占位符/方块。

使用 `scripts/chinese_font.py` 中的工具函数：
```python
from scripts.chinese_font import set_default_chinese_font, set_chinese_font

doc = Document()
set_default_chinese_font(doc, '宋体', 12)  # 设置全文档默认字体
```

常用中文字体：宋体、黑体、仿宋、楷体、微软雅黑

原理：python-docx 默认字体为 Calibri（西文字体），需通过 `w:eastAsia` 指定东亚字体。
